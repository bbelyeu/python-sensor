...
class APIRequest(object):
    ...
    def post(self, endpoint, payload):
        agent_key_64 = base64_encode(self.apagent.get_option('agent_key') + ':').replace('\n', '')
        headers = {
        ...
        req_body = {
            ...
            'agent_version':   self.apagent.AGENT_VERSION,
            'app_name':        self.apagent.get_option('app_name'),
            'app_version':     self.apagent.get_option('app_version'),
            'app_environment': self.apagent.get_option('app_environment'),
            'host_name':       self.apagent.get_option('host_name', host_name),
        ...
        request = Request(
            url = self.apagent.get_option('dashboard_address') + '/agent/v1/' + endpoint,
            data = req_body_gzip,
            headers = headers)
