...
class AllocationProfiler(object):
    ...
    def setup(self):
        if self.apagent.get_option('allocation_profiler_disabled'):
            return

        if not runtime_info.OS_LINUX and not runtime_info.OS_DARWIN:
            self.apagent.log('CPU profiler is only supported on Linux and OS X.')
            return

        if not min_version(3, 4):
            self.apagent.log('Memory allocation profiling is available for Python 3.4 or higher')
            return
    ...
    def start_profiler(self):
        self.apagent.log('AutoProfile Activating memory allocation profiler.')

        def start():
            tracemalloc.start(self.MAX_TRACEBACK_SIZE)
        self.apagent.run_in_main_thread(start)

        self.start_ts = time.time()

        def monitor_overhead():
            if tracemalloc.is_tracing() and tracemalloc.get_tracemalloc_memory() > self.MAX_MEMORY_OVERHEAD:
                self.apagent.log('Allocation profiler memory overhead limit exceeded: {0} bytes'.format(tracemalloc.get_tracemalloc_memory()))
                self.stop_profiler()

        self.overhead_monitor = self.apagent.schedule(0.5, 0.5, monitor_overhead)


    def stop_profiler(self):
        self.apagent.log('AutoProfile Deactivating memory allocation profiler.')
        ...
        with self.profile_lock:
            ...
            if tracemalloc.is_tracing():
                ...
                self.apagent.log('Allocation profiler memory overhead {0} bytes'.format(tracemalloc.get_tracemalloc_memory()))
                tracemalloc.stop()
        ...
        for stat in stats[:self.MAX_PROFILED_ALLOCATIONS]:
            if stat.traceback:
                ...
                for frame in stat.traceback:
                    if self.apagent.frame_cache.is_agent_frame(frame.filename):
                        skip_stack = True
                        break
                if skip_stack:
                    continue
