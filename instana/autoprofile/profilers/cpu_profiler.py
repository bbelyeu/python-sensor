...
class CPUProfiler(object):
    ...
    def setup(self):
        if self.apagent.get_option('cpu_profiler_disabled'):
            return

        if not runtime_info.OS_LINUX and not runtime_info.OS_DARWIN:
            self.apagent.log('AutoProfile CPU profiler is only supported on Linux and OS X.')
            return
    ...
    def start_profiler(self):
        self.apagent.log('AutoProfile Activating CPU profiler.')
