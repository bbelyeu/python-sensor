...
class AutoProfileAgent(object):
    ...
    def start(self, **kwargs):
        ...
        if self.agent_destroyed:
            self.log('Destroyed AutoProfile agent cannot be started')
            return
        ...
        self.log('AutoProfile agent started')
    ...
    def destroy(self):
        if not self.agent_started:
            self.log('AutoProfile agent has not been started')
            return
    ...
    def log_prefix(self):
        return '[' + datetime.datetime.now().strftime('%H:%M:%S.%f') + '] Instana AutoProfile ' + self.AGENT_VERSION + ':'
    ...
    def exception(self):
        # if self.get_option('debug'):
        logger.debug("AutoProfile exception")
