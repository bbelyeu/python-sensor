...
class ErrorReporter(object):
    ...
    def setup(self):
        if self.apagent.get_option('error_profiler_disabled'):
            return

        def _exc_info(args, kwargs, ret, data):
            try:
                if not self.apagent.agent_started or self.apagent.agent_destroyed:
                    return
                ...
                self.apagent.log('exc_info wrapper exception')
    ...
    def start(self):
        ...
        if not self.apagent.get_option('auto_profiling'):
            return
        ...
        self.process_timer = self.apagent.schedule(1, 1, self.process)
        self.report_timer = self.apagent.schedule(60, 60, self.report)
    ...
    def report(self):
        with self.profile_lock:
            ...
            metric = Metric(self.apagent, Metric.TYPE_PROFILE, Metric.CATEGORY_ERROR_PROFILE, Metric.NAME_HANDLED_EXCEPTIONS, Metric.UNIT_NONE)
    ...
    def recover_stack(self, exc):
        ...
        for tb_frame in tb_stack:
            ...
            if self.apagent.frame_cache.is_agent_frame(filename):
                return None

            if not self.apagent.frame_cache.is_system_frame(filename):
