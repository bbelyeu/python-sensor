...
class ProfileReporter:
        ...
        if self.apagent.get_option('auto_profiling'):
            if not self.config.report_only:
                def random_delay():
                    ...
                    self.random_timer = self.apagent.delay(timeout, self.start_profiling, False, True)

                self.span_timer = self.apagent.schedule(0, self.config.span_interval, random_delay)

            self.report_timer = self.apagent.schedule(self.config.report_interval, self.config.report_interval, self.report)
    ...
    def start_profiling(self, api_call, with_timeout):
        ...
        if self.profile_duration > self.config.max_profile_duration:
            self.apagent.log(self.config.log_prefix + ': max profiling duration reached.')
            return False

        if api_call and self.span_count > self.config.max_span_count:
            self.apagent.log(self.config.log_prefix + ': max recording count reached.')
            return False

        if self.apagent.profiler_active:
            self.apagent.log(self.config.log_prefix + ': profiler lock exists.')
            return False

        self.apagent.profiler_active = True
        self.apagent.log(self.config.log_prefix + ': started.')

        try:
            self.profiler.start_profiler()
        except Exception:
            self.apagent.profiler_active = False
            self.exception()
            return False

        if with_timeout:
            self.span_timeout = self.apagent.delay(self.config.max_span_duration, self.stop_profiling)
    ...
    def stop_profiling(self):
        ...
        self.apagent.log(self.config.log_prefix + ': stopped.')
    ...
    def report(self, with_interval=False):
        ...
        self.apagent.log(self.config.log_prefix + ': reporting profile.')

        profile_data = self.profiler.build_profile(self.profile_duration)

        for data in profile_data:
          metric = Metric(self.apagent, Metric.TYPE_PROFILE, data['category'], data['name'], data['unit'])
          metric.create_measurement(self.span_trigger, data['profile'].measurement, data['unit_interval'], data['profile'])
          self.apagent.message_queue.add('metric', metric.to_dict())
